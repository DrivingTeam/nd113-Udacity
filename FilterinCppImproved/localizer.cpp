...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
  ...
  newGrid.assign(grid.size(), vector <float> (grid[0].size(), 1.0 / size));
	// your code here
	...
	float p_miss) 
{
  ...
  assert(grid.size() == beliefs.size());
  assert(grid[0].size() == beliefs[0].size());
  // newGrid = zeros(grid.size(), grid[0].size());
  for(int i=0;i<newGrid.size();i++) 
    for(int j=0;j<newGrid[0].size();j++) 
      newGrid[i][j] = (beliefs[i][j] * \
        (p_hit * color == grid[i][j] + p_miss * (1 - color == grid[i][j])));
	// your code here
