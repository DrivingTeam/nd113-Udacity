...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	float centre = 1 - blurring;
	// vector <float> row1 = {corner, adjacent, corner};
	// vector <float> row2 = {adjacent, centre, adjacent};
	// vector <float> row3 = {corner, adjacent, corner};
	window.assign({{corner, adjacent, corner}, {adjacent, centre, adjacent}, {corner, adjacent, corner}});
	vector < vector <float> > newGrid(grid.size(), vector <float> (grid[0].size(),0));
	for(int i=0;i<newGrid.size();++i)
		for(int j=0;j<newGrid[0].size();++j)
			for(int dy=-1;dy<=1;++dy)
				for(int dx=-1;dx<=1;++dx) 
					newGrid[i][j] += window[dy+1][dx+1] * \
				grid[(i+dy+newGrid.size())%newGrid.size()]\
				[(j+dx+newGrid[0].size())%newGrid[0].size()];
